        ...
        <div class="container mx-auto px-4 max-w-6xl">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div class="lg:col-span-2 space-y-6">
                    ...
                    @if (!empty($availabilityCalendar) && $availabilityCalendar->isNotEmpty())
                        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
                            <h2 class="text-xl font-semibold text-gray-900 dark:text-white mb-4">Availability Calendar</h2>
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                @foreach ($availabilityCalendar as $date => $item)
                                    <div class="rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                                        <div class="flex items-center justify-between mb-2">
                                            <p class="font-medium text-gray-900 dark:text-white">{{ \Carbon\Carbon::parse($date)->format('d M Y') }}</p>
                                            <p class="text-sm font-semibold {{ $item['remaining'] > 0 ? 'text-teal-600' : 'text-red-500' }}">{{ $item['remaining'] }} {{ $product->type === 'car' ? 'cars' : 'seats' }}</p>
                                        </div>
                                        <div class="flex flex-wrap gap-2">
                                            @foreach ($item['slots'] as $slot)
                                                <span class="text-xs px-2 py-1 rounded bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300">
                                                    {{ $slot['time'] }} ({{ $slot['remaining_capacity'] }})
                                                </span>
                                            @endforeach
                                        </div>
                                    </div>
